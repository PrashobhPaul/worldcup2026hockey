...
Prefer a classic install? The **[Android APK](https://github.com/PrashobhPaul/worldcup2026hockey/releases/download/apk-latest/HockeyAI.apk)** wraps the very same app in a native shell (a Trusted Web Activity, built by this repository's own CI) — the link downloads `HockeyAI.apk` directly; allow the install when Android asks, done. Either way: follow a team on its page and tap the bell on Home to get start-of-match and full-time notifications while the app is open or in the background. A zero-backend app has no push server, so a fully closed app stays silent — by design, nothing can track you to wake you.
...
- `scripts/model_non_knockout.py` — the pick rule for the forty matches that can be drawn, ported from the model author's `reference/model_v3/non_knockout_model_v2.py`. The ranking-points base produces the probabilities; on top of it sit the draw rule (two sides converged to within a hundred ranking points of each other) and the five tournament rules — favourite live-surge, base-draw divergence, head-to-head override, ranking-gap band and underdog goal difference. Which set is published is `mode` in `model/params.json`; `scripts/test_non_knockout_model.py` holds the port to the author's reference replay in both modes and checks every published pick against it.

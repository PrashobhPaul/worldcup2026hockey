...
Kane Russell settled it in the 49th, converting the corner that made it four, and Abdul Manan's green card in the 51st was the last incident of note. Both of Pakistan's goals from the set piece, all four of New Zealand's from open play and one corner — the same split that has defined both campaigns. New Zealand finish Stage 2 with three wins from five; Pakistan, with thirteen scored and nineteen conceded, leave a tournament in which they were never dull and never quite solid.""",
...
Joel Rintala pulled Australia back to 3-2 from open play in the 30th, on the stroke of half-time, and the match Australia needed was there again. It lasted ten minutes: Hugo von Montgelas converted Germany's only penalty corner of the evening in the 40th, and from two goals up Germany managed the game home, with Müller's green card in the 50th the only late interruption.
...
REASONS = {
    ...
    'S2E3': "Argentina's only defeat in four matches this tournament is to the Netherlands, and they arrive from beating England 3-1 on sixty-one per cent of possession and three penalty-corner goals. India's four matches split the other way: wins over Wales and Pakistan, defeats in both meetings with top-eight opposition, and eleven conceded overall — nearly three a match. Argentina's corner battery against the tournament's most breached top-eight defence favours Los Leones.",

    'S2F3': "Australia are the only side left in Pool F yet to lose a match this tournament — two wins and two draws around the pool's stingiest defence, four conceded in four. Germany's scoring has thinned since the five they put past Malaysia: one goal per match in the three games since, and Spain have just ended their unbeaten run. An attack producing once a game against the steadiest defence in the pool is the harder road; Australia are favoured to stay unbeaten.",
    ...
    'S2G4': "New Zealand's defeats have come only against the Netherlands and Argentina; outside the top eight they have won every match they have played, the Wales rout showing an attack that punishes a soft defence. Pakistan's eleven goals in four matches say they will score, but they have conceded at least three in every game, and their only win came against a Japan side that had scored once all tournament before that afternoon. New Zealand's edge at both ends is favoured.",

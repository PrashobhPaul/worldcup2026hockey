...
def engine_brief(m, teams):
    ...
    if hg or ag:
        ...
        p2 = 'A detailed scorer breakdown for this match is not in the feed yet.'

    # Paragraph 3 — the shape of the game.
    p3_bits = []
    if pc.get('home') is not None and pc.get('away') is not None:
        p3_bits.append(f"{hn} won {pc['home']} penalty corners to {an}'s {pc['away']}")
    ...
    p3 = (p3 + ' This brief is generated by the Hockey.AI engine directly from the match ledger.').strip()

...
front of it. Every brief below was written from the full match ledger — goals,
methods, minutes, penalty corners, cards — with the tournament to that point in
view, so the 24 read as one series rather than 24 unrelated paragraphs, and so
each one can say what actually decided the game instead of listing what
happened in it.
...
shown IN THIS TOURNAMENT up to the previous match — its scoring pattern, its
penalty-corner economy, its discipline — and names the strongest evidence
...
# says about the two sides. Every number below is in the ledger.
STORIES = {
    'D1': """India opened their World Cup by turning one weapon on Wales and never putting it down. All three goals came from penalty corners, three of eight won, and by the time Wales found an answer the game had already been decided by a routine they ran better than their opponents could defend it.
...
The corner count — eight to two — is the honest measure of territory here. Wales defended the run of play competently enough to keep the score respectable, but conceding four short-corner opportunities for every one they won against a battery of this quality left them chasing a game they never led. India's set piece is a genuine tournament weapon; their open play, on this evidence, still needs one.""",
...
The corner count was almost level, eight to seven, and that is the number Pakistan should take from this. They generated real pressure and won short corners against a top-five defence; they converted none of them. A green card in the 20th, immediately before England's second and third goals, did not help. On this evidence Pakistan's problem in this tournament is conversion, not creation.""",

    'B2': """France led this match twice and still lost it, which tells you exactly where Belgium's advantage lies: in the last sixteen minutes, and in the circle. Two penalty corners late turned a game France had shaped for most of an hour.
...
Belgium won eight corners to six and scored from the two that mattered; France won six and converted none. That is the entire margin. For France there is real encouragement here — they scored twice against the world's top-ranked side and led them for a quarter of the match — but a team that cannot punish short corners at this level is asking to be beaten late, and was.""",

    'C1': """Australia and Ireland won five penalty corners each. Australia scored three of theirs; Ireland scored one. On a night when both sides earned identical opportunities, the difference between them was entirely in the execution, and Australia's was ruthless.
...
Corners were level at six each and Spain scored none of theirs, which makes the result more impressive rather than less: this was a win built entirely in open play against a side that defends the set piece well. South Africa's counter-attack is genuine and Cassiem is a real finisher, but conceding three inside twenty-eight minutes leaves too much to do.""",
...
Six corners to three understates the Dutch grip on the second half. What should concern the rest of Pool A is not the margin but the shape of it — the Netherlands were behind, in front of their own crowd, and their response was to score at will rather than to force set pieces. New Zealand defended one half competently and were dismantled in the other.""",
...
Argentina won five corners to three and scored from none of them, which is worth noting: Los Leones did all of this without their set piece firing. Japan will take the shape of the first three quarters as something to build on, but a side that creates nothing at the other end is only ever one lapse from exactly this ending.""",
...
France won eight corners to five and converted one; Malaysia won five and converted one, and scored their other two from open play. The card count tells the rest — five greens between them, three to France in a six-minute spell in the second quarter — of a match played at a pace neither side could control. Both took a point that flatters and frustrates in equal measure: Malaysia for leading three times, France for needing the sixtieth minute to avoid defeat.""",

    'D3': """India won the penalty corner count and lost the match. Eight short corners to England's seven yielded a single goal, while England scored three times from open play and once from a set piece, and that contrast is the entire ninety-minute argument.
...
England took four green cards in this match and still controlled the second half, which speaks to how comfortable they were without the ball. India led at half-time against the tournament's most in-form attack and were outscored 3-0 after it. Their set piece remains the best in the competition; their inability to score from open play against organised defences is now a pattern, not an accident.""",
    ...
    'B3': """One penalty corner, one goal, one result. Germany won the only short corner of the match and Jakob Brilla converted it in the 39th minute; Belgium, the top-ranked side in the world and the most feared set-piece unit in the tournament, did not win a single one.

That statistic is not a curiosity, it is the match report. Belgium were denied the entry passes and the circle contacts that generate corners, and without them Alexander Hendrickx — the most dangerous flick in the competition — never took one. Germany defended the top of their circle for sixty minutes and attacked it once, successfully.
...
Japan have now played two matches without scoring and without earning a corner in this one, which is the harder of the two problems. New Zealand, beaten 5-1 in their opener, will take a clean sheet and a first win, achieved by playing within themselves. Neither performance suggests a side about to trouble the top of Pool A, but New Zealand's was the more complete.""",
    ...
    'C3': """The best defensive contest of the pool stage: no penalty corners, six cards, and a single penalty stroke to separate two sides who had each won their opener by the same 3-1 scoreline. Blake Govers converted in the 47th minute and Australia won a match in which neither team earned a short corner.

That is worth restating — sixty minutes between the fourth and sixth ranked sides in the world produced zero corners. Both defences protected the top of the circle rather than the goal line, forcing errors outside it. Australia collected three green cards in the first twenty-three minutes, Spain two more in the second half, and Tim Brand's yellow in the 54th made the closing minutes genuinely uncomfortable.
...
Kenton Melville put South Africa ahead in the 38th from open play. Benjamin Walker levelled in the 43rd, then Lee Cole converted both of Ireland's penalty corners — the 49th and the 56th — and Sam Hyland scored with the last action of the match in the 60th. Two corners won, two goals: the most efficient set-piece return of the entire pool stage.

South Africa's discipline unravelled first. A yellow in the 10th, greens in the 15th and 60th, and no corners of their own in the whole match meant that once they lost the lead they had no route back into it. Ireland found in Cole a set-piece answer to the problem that cost them against Australia, and turned a losing position into a four-goal haul without ever looking hurried.""",
...
Harmanpreet Singh converted in the 5th and again in the 21st — India's only two penalty corners of the match, both scored. Abhishek added field goals in the 11th and the 24th. Pakistan refused to fold: Hannan Shahid in the 22nd, Sufyan Khan from a corner in the 24th, and Shahid again in the 46th. Hardik Singh's penalty stroke in the 35th proved the decisive fifth.

Two corners, two goals is perfect conversion and a marked change from the eight-for-one against England. India's open-play scoring — two from Abhishek — is the more meaningful development, because it answers the criticism that has followed them through the pool. Pakistan scored three times against a top-eight defence and still lost; their defending of the counter, not their attack, is what ends their pool campaign.""",
...
South Africa won more corners than Australia, two to one, having won none at all against Ireland. Cassiem's conversion rate is the story of their tournament — he has now scored from set pieces in three consecutive matches — but so is the discipline: two yellows and two greens, and a side that spent long spells short-handed and defended through them. Australia drop their first points of the tournament to the lowest-ranked team in Pool C.""",
...
Both of Argentina's penalty corners were converted; New Zealand won none. Four of the seven goals came from open play, which is the number that matters — this was not a set-piece exhibition but a side scoring in every way available to it. New Zealand, who had kept Japan out four days earlier, conceded more here than in their previous two matches combined.""",
...
One corner won, one scored, for the second time in this pool stage that a perfect set-piece return has decided a match. Ireland created nothing from short corners because they earned none, and against a Spanish defence that concedes very few, that leaves only open play and strokes. Spain finish the pool with two wins and a single-goal defeat to Australia, which is the profile of a side nobody wants in their Stage 2 group.""",
    ...
    'A5': """Japan scored their first goal of the tournament and led the hosts for four minutes. The Netherlands then won two penalty corners and scored from both, added a stroke, and finished with four goals in a match they had made hard work of for fifty minutes.
...
Two corners, two goals is the second perfect set-piece return of the day and the pattern of the Dutch pool campaign: they have now scored twelve goals across three matches, and in every one of them the decisive scoring has come after the interval. Japan leave the pool stage with one goal and no points, but with sixty minutes here that bore no resemblance to their first two matches.""",
}
...
REASONS = {
    'D1': "Opening match, so there is no tournament form to read yet: the case rests on India's penalty-corner battery, with Harmanpreet Singh the most dangerous drag flick either side can put on the pitch, against a Welsh defence that will have to survive repeated short corners to stay in it. The risk is that India have historically been corner-dependent, and one good defensive routine can keep this close.",
    ...
    'C1': "Nothing played yet on either side. Australia's transition speed is the sharpest weapon on show and their set-piece unit converts at a rate few defences survive; Ireland's answer is organisation rather than pace, and they defend the circle well enough to keep the corner count level. That is exactly the scenario in which this becomes tight — if Ireland match Australia for short corners, execution decides it.",
    ...
    'D4': "Both lost their openers, and both lost them differently: Pakistan won seven penalty corners against England and converted none, while Wales were held to two corners by India and scored only in the 56th minute. Pakistan's ability to manufacture set pieces at this level is the edge here. Their discipline is the doubt — they took a green card against England, and this fixture rarely stays calm.",

    'B4': "Malaysia's five penalty corners against Germany and Shello Silverius's willingness to shoot on sight give them a route to goal that does not depend on sustained possession, which matters against a French side that plays through the middle. The strong counter-argument is France's opener: they scored twice against the top-ranked side in the world and led them twice, which is more attacking evidence than Malaysia's 1-5 provides.",

    'D3': "India come in having won eight penalty corners against Wales and scored three, the best set-piece return of the opening round, and Harmanpreet Singh's two goals make them the more dangerous side the moment the ball enters the circle. England's counter-case is real and it is about breadth — four different scorers against Pakistan, in all four quarters, from both open play and set pieces, which is a harder pattern to defend than a single routine.",
    ...
    'A4': "Both lost their openers, but not equally: New Zealand scored first against the Netherlands and had the game level at half-time before it ran away, while Japan were kept scoreless by Argentina and won only three short corners. New Zealand have shown they can generate goals against a top-two defence; Japan have yet to show they can generate any, and that gap decides an evenly matched fixture.",

    'C3': "Both sides won their openers 3-1, which makes the method the deciding evidence: Australia scored all three of theirs from penalty corners at sixty per cent conversion, while Spain scored all three from open play. Against a Spanish defence that gave up six corners to South Africa, the Australian set piece is the likelier route to goal. The reason this is close to a coin toss is that Spain conceded only one and never needed a set piece themselves.",

    'C4': "Both lost their openers by the same 1-3 scoreline, but Ireland matched Australia for penalty corners won, five each, while South Africa conceded three inside twenty-eight minutes to Spain and took two green cards in the closing eight minutes. Ireland's problem is conversion, not creation — one goal from five corners — and against a South African side that struggles to hold a lead, the side that keeps generating chances is favoured.",

    'A3': "The Netherlands have scored five and conceded one, and did their scoring in an eighteen-minute burst after falling behind, which is the profile of a side that does not panic. Argentina's clean sheet against Japan is the counter-evidence and a good one — they conceded nothing and controlled the middle quarters — but they created only from open play and won five corners without scoring from any.",
    ...
    'D6': "India have scored five in two matches and won ten penalty corners across them, with Harmanpreet Singh converting three; Pakistan have one point, seven goals conceded, and a discipline record — a yellow and three greens against Wales — that hands a set-piece side extra opportunities. Pakistan's three goals in that draw prove they can score against anyone in this pool, which is why the margin is less certain than the outcome.",

    'B6': "Germany have won both matches, conceded once, and beaten Belgium by denying them a single penalty corner — the most complete defensive performance of the pool stage. France have taken one point from two and needed a 59th-minute equaliser to get it. The caution is that France have now scored in every match and against Belgium showed they can punish one lapse, and Germany's wins have both been narrow in chances if not in scoreline.",

    'B5': "Belgium's defeat to Germany came without a single penalty corner won, and against Malaysia — who conceded nine to Germany and five to France — they should get their set piece back, with Alexander Hendrickx and Tom Boon both capable of scoring from it. Malaysia's counter-argument is their scoring: they took three off France, converting from short corners in both matches, and Belgium's defence has now been breached in both of theirs.",

    'C5': "Australia have six points, four goals and one conceded, and have scored from a corner, from open play and from a stroke — three routes to goal against a South African side that has conceded seven in two matches and won no penalty corners at all against Ireland. The one warning is discipline: Australia took three green cards against Spain in twenty-three minutes, and South Africa's Mustaphaa Cassiem has scored in both matches.",

    'A6': "Argentina beat Japan without conceding and lost to the Netherlands only after a yellow card gave the hosts the opening they needed; New Zealand have beaten Japan and been beaten 5-1 by the same Dutch side. The two results overlap almost exactly, so the edge is Argentina's attacking variety — five corners won in one match, three different scorers in another — against a New Zealand side that has scored three in two.",

    'C6': "Spain's only defeat came 1-0 to Australia in a match with no penalty corners at all, which says their defensive shape travels; Ireland's four goals against South Africa came from a side that had lost its opener and needed twenty-two minutes to turn the game. Ireland's Lee Cole converted both corners in that win, so the set piece is a genuine threat — but Spain have conceded two goals in two matches and give up very little in the circle.",

    'A5': "The Netherlands have won both matches, scored eight and taken the second half of each one decisively; Japan have not scored in the tournament, have not won a penalty corner in their last match, and have conceded five. Japan's defensive shape kept Argentina to one goal for thirty-three minutes, so a low-scoring first half is plausible — but a side with no attacking output cannot hold this Dutch forward line for sixty.",
}
